...
public static class StructuralJsonDiff
{
    ...
    {
        var root = Build(oldEl, newEl, "$", "$");
        Prune(root);
        return root;
    }

    private static JsonDiffNode Build(JsonElement oldEl, JsonElement newEl, string path, string name)
    {
        ...
        switch (oldEl.ValueKind)
        {
            case JsonValueKind.Object:
                return BuildObject(oldEl, newEl, path, name);
            case JsonValueKind.Array:
                return BuildArray(oldEl, newEl, path, name);
            default:
    ...
    private static JsonDiffNode BuildObject(JsonElement oldEl, JsonElement newEl, string path, string name)
    {
        var node = new JsonDiffNode { Path = path, Name = name, ChangeType = JsonDiffChangeType.Unchanged };
        ...
        foreach (var key in keys)
        {
            ...
            if (inOld && inNew)
                node.Children.Add(Build(oldChild, newChild, childPath, key));
            else if (inOld)
                node.Children.Add(Leaf(childPath, key, JsonDiffChangeType.Removed, Stringify(oldChild), null));
            else
                node.Children.Add(Leaf(childPath, key, JsonDiffChangeType.Added, null, Stringify(newChild)));
        }

        return node;
    }

    private static JsonDiffNode BuildArray(JsonElement oldEl, JsonElement newEl, string path, string name)
    {
        var node = new JsonDiffNode { Path = path, Name = name, ChangeType = JsonDiffChangeType.Unchanged };
        ...
        for (var i = 0; i < Math.Max(oldItems.Count, newItems.Count); i++)
        {
            ...
            if (i < oldItems.Count && i < newItems.Count)
                node.Children.Add(Build(oldItems[i], newItems[i], childPath, childName));
            else if (i < oldItems.Count)
                node.Children.Add(Leaf(childPath, childName, JsonDiffChangeType.Removed, Stringify(oldItems[i]), null));
            else
                node.Children.Add(Leaf(childPath, childName, JsonDiffChangeType.Added, null, Stringify(newItems[i])));
        }

        return node;
    }

    private static JsonDiffNode Leaf(string path, string name, JsonDiffChangeType type, string? oldValue, string? newValue)
        => new() { Path = path, Name = name, ChangeType = type, OldValue = oldValue, NewValue = newValue };
